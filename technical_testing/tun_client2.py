...
def tun_create(if_name, ipaddress='172.16.10.1/24', brd='172.16.10.255', gateway='', owner=0):
    ...
    subprocess.check_call('ip addr add dev {} {} brd {}'.format(
        if_name, ipaddress, brd), shell=1)
    if gateway != '':
        subprocess.check_call('ip route change {} via {}'.format(
            ipaddress, gateway), shell=1)
    return tun
...
tun = tun_create(
    if_name='tun0', ipaddress='{}/24'.format(socket.inet_ntoa(data)))
fd = tun.fileno()
...
def tun_send():
    while 1:
        ...
        if send_count != len(data):
            print('Error send :', 'send size',
                  send_count, 'data size', len(data))
